...
By default if nothing is specified in a source connection string to an upstream filter, all topics are subscribed to (with the exception of special system topics like `_metrics` and `_filter`, explained below). This means that you don't have to know what the upstream filter publishes, you will get all of it. Such a `sources` string might take the format of `tcp://upstream_filter:5554`.
...
In addition to the normal topics filters publish, there are system topics that (unless explicitly turned off with config options) are published individually by each filter and available for downstream filters to subscribe to explicitly only - the `_metrics` topic with performance metrics, and the `_filter` topic with frame ID information for traceability. There is also a way to subscribe to EVERYTHING from upstream as if you didn't specify any explicit topics at all which will include these system topics, simply subscribe to `*` as in `tcp://upstream_filter:5556;*`. Don't subscribe to anything else, just this, and you will get EVERYTHING from the upstream filter, including any future system level topics that might start with `_` and be undelivered otherwise like the `_metrics` and `_filter` topics.
